...
//        'query' => 'SELECT uid, givenName, email, eduPersonPrincipalName FROM users WHERE uid = :username AND password = SHA2(CONCAT((SELECT salt FROM users WHERE uid = :username), :password), 256);',
        ...
        'query' => 'SELECT 
              ...
              U.PASSWORD, 
              U.EXPIRATION_DATE,
              U.EXPIRATION_DATE - sysdate as EXPIREDELTA,
              U.USER_PROFILE_ID, 
              U.STATUS, 
              U.ERROR_COUNTER, 
              U.EMAIL,
              AU.AGENT_ID, 
              A.TRADE_NAME, 
              NVL(A.LEVEL_ID,S.LEVEL_ID) AS LEVEL_ID, 
              SU.SUBSCRIBER_ID, 
              S.MSISDN,
              AB.BRAND_ID AS AGENT_BRAND, 
              S.BRAND_ID AS SUBSCRIBER_BRAND, 
              U.LAST_ACCESS
              FROM USER_ADM U
              LEFT JOIN AGENT_USER AU ON U.USER_ID = AU.USER_ID
              LEFT JOIN AGENT A ON AU.AGENT_ID = A.AGENT_ID
              LEFT JOIN AGENT_BRAND AB ON AU.AGENT_ID = AB.AGENT_ID
              LEFT JOIN SUBSCRIBER_USER SU ON U.USER_ID = SU.USER_ID
